...
def gen_img(directory, samples_dir, cending, ccapt, ccast, cpromo, suffix):
    ...
    cast_imgs_paths=[get_rand_img(directory, castling[i]) for i in range(len(castling))]
    ...
    cast_imgs=[cv2.imread(cast_imgs_paths[i]) for i in range(len(castling))]

    if ccast:
        combined_img=combine_imgs([cast_imgs[0], cast_imgs[1], cast_imgs[2]])
        base_name=f"{castling[0]}{castling[1]}{castling[2]}"
    elif cpromo and ((rank == '1' or rank == '8') and not piece == 'K'):
